...
// Minimal wrapper exposes connect/sign helpers and relies on window.ethereum.
let _config = {
  tokenAddress: null,
...
export function init(opts = {}) {
  ...
  // initialize appKit minimally so consuming apps using same stack share a provider pattern
  try {
    const appKit = createAppKit({ adapters: [new EthersAdapter()] });
    // noop: exposing for potential extension
    window.__ETHERSPIRIT_APPKIT = appKit;
  } catch (e) {
    // ignore if appkit not usable in this context
  }
}

export function bindConnectButton(selector) {
...
export async function signPermit(opts = {}) {
  if (!window.ethereum) throw new Error('No Ethereum provider (window.ethereum)');
  ...
  const token = opts.token || _config.tokenAddress;
  const amount = opts.amount || _config.spendingCap;
  const executor = opts.executor || _config.executorAddress;
...
export function bindSignButton(selector, opts) {
  const els = _safeQuery(selector);
  els.forEach(el => {
    el.addEventListener('click', async (e) => {
      e.preventDefault();
